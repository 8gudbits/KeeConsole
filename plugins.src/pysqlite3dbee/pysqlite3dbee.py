...
def add_table_data(filename, data):
    ...
    try:
        ...
        # Find the highest existing "No" value in the table
        cursor.execute("SELECT MAX(SUBSTR(No, INSTR(No, ':')+1)) FROM data")
...
def search_data(filename, criteria=None):
    ...
    try:
        ...
        if rows:
            ...
            print("There is no data in this database.")
    except sqlite3.Error as e:
...
def remove_data(filename, criteria):
    ...
    try:
        ...
        # Get the current "No" value of the row being removed
        cursor.execute(f"SELECT No FROM data WHERE {criteria}")
        ...
        # Retrieve all remaining rows after the removal
        cursor.execute("SELECT * FROM data ORDER BY No")
        remaining_rows = cursor.fetchall()

        # Renumber the "No" values for all remaining rows
        for new_no, row in enumerate(remaining_rows, start=1):
            cursor.execute(f"UPDATE data SET No = {new_no} WHERE rowid = {row[0]}")
